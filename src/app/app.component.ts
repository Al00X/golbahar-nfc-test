...
export class AppComponent {
  ...
  constructor(
    ...
    private sqliteWrapper: SqliteWrapperService
  ) {
          ...
          .then(async (ret) => {
            if (this.sqliteWrapper.platform === 'web') {
              ...
              if (jeepSqliteEl != null) {
                ...
                console.log(
                  `>>>> isStoreOpen ${await jeepSqliteEl.isStoreOpen()}`
                );
              } else {
            ...
            if (!(await this.sqlite.isConnected())) {
              console.log('INITING CONNECTION');
              await this.sqlite.getOrCreateConnection(this.DATABASE_NAME).then(() => {
                console.log(
                  `Sqlite is connected to the database: ${this.DATABASE_NAME}`
                );
              });
            }
